@php
  ...
  $searchAction = $hasArtisan
      ...
      : ($hasHotel ? route('partners.bookings.index') : route('partners.dashboard'));
...
<div class="header">
  <div class="main-header">
        ...
        <div class="header-left w-100 text-center">
            <a href="{{ route('home') }}"
            class="logo-text logo logo-normal d-inline-flex align-items-center justify-content-center"
            target="_blank"
            rel="noopener">
                Accueil
                <i class="ti ti-external-link ms-1 fs-12"></i>
            </a>
        </div>
    ...
    <div class="header-user">
      <div class="nav user-menu nav-list">
        ...
        @if(session()->has('impersonator_id'))
            <div class="alert alert-danger text-center mt-1 mb-2 p-1">
                Vous êtes connecté en tant que <strong>{{ auth()->user()->firstname }} {{ auth()->user()->lastname }}</strong>.
                <form action="{{ route('impersonate.stop') }}" method="POST" class="d-inline">
                    @csrf
                    <button class="btn btn-sm btn-dark ms-2">Revenir à mon compte</button>
                </form>
            </div>
        @endif
