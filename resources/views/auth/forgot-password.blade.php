...
<head>
  <meta charset="utf-8">
  <title>Mot de passe oublié • MYLMARK</title>
  ...
  {{-- Open Graph --}}
  <meta property="og:title" content="@yield('og_title', 'Mot de passe oublié • MYLMARK')">
  <meta property="og:description" content="@yield('og_desc', 'Demandez un lien sécurisé pour réinitialiser le mot de passe de votre compte MYLMARK et retrouver l’accès à vos commandes et favoris.')">
  ...
  {{-- SEO classique --}}
  <meta name="description" content="Vous avez oublié votre mot de passe MYLMARK ? Demandez un lien de réinitialisation pour sécuriser et retrouver l’accès à votre compte en quelques instants.">
  <meta name="keywords" content="MYLMARK, mot de passe oublié, réinitialisation, compte client, connexion, sécurité, marketplace africaine, artisans, créateurs">
    ...
    <div class="max-w-lg mx-auto px-4 sm:px-6">
      ...
      @if ($errors->any())
        <div class="mb-4 rounded-xl border border-amber-300/40 bg-amber-100/10 text-amber-100 px-4 py-3">
          Merci de vérifier le champ ci-dessous.
        </div>
      @endif

      <form method="POST" action="{{ route('password.email') }}" novalidate class="card overflow-hidden">
        @csrf
        ...
        <div class="p-6 sm:p-8 space-y-4">
          <div>
            ...
            <input  id="email" name="email"
              type="email"  class="inp"  value="{{ old('email') }}"
              placeholder="exemple@email.com"  autocomplete="email" required autofocus
            >
        ...
        <div class="px-6 sm:px-8 pb-6 sm:pb-8">
          <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:justify-between">
            ...
            <button class="btn text-center " type="submit">Envoyer le lien</button>
          </div>
          <p class="mt-3 text-xs text-white/60">
            Note : pensez à vérifier votre “spam” ou “indésirables”.
          </p>
        </div>
      </form>
    </div>
