...
<div class="row g-3">
  ...
  <div class="col-md-4">
    ...
    <input type="text" name="unit" class="form-control"
           value="{{ old('unit', $product->unit ?? '') }}" placeholder="Ex : kg / cm / m">
  </div>
...
<div id="variants" class="{{ old('type', $product->type ?? 'simple')==='variable' ? '' : 'd-none' }}">
  ...
  <div class="border rounded">
    <table class="table table-sm align-middle mb-0" id="skuTable" style="min-width: 980px">
      ...
      <thead>
        <tr>
          <th>Attributs (clé=valeur; clé2=val2)</th>
          <th>SKU</th>
          <th>Prix</th>
          <th>Ancien Prix</th>
          <th>Stock</th>
      ...
      <tbody>
        ...
        @foreach($rows as $i => $row)
          <tr>
            ...
            <td><input type="number" step="0.01" name="skus[{{ $i }}][price]" class="form-control"
                       value="{{ $row['price'] ?? '' }}" placeholder="Ex : 149000"></td>
            <td><input type="number" step="0.01" name="skus[{{ $i }}][old_price]" class="form-control"
            ...
            <td><input type="text" name="skus[{{ $i }}][unit]" class="form-control"
                       value="{{ $row['unit'] ?? ($product->unit ?? '') }}" placeholder="Ex : kg / cm / m"></td>
...
<script>
    ...
    window.addSkuRow = function(){
        ...
        <td><input type="text" name="skus[${idx}][unit]" class="form-control" value="${DEFAULT_UNIT || ''}" placeholder="Ex : kg / cm / m"></td>
    ...
    window.deleteMedia = async function(productId, mediaId, btn){
        ...
        const res = await fetch(`{{ url('/partners/products') }}/${productId}/media/${mediaId}`, {
        method: 'DELETE',
