...
<div class="mb-2">
  <a href="{{ route('partners.shop.orders.index') }}" class="btn btn-sm btn-light">
...
<div class="row g-3">
      ...
      <div class="card-body">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <div class="d-flex justify-content-between"><span>Sous-total</span><span>{{ number_format($order->subtotal,0,' ',' ') }} </span></div>
            {{-- <div class="d-flex justify-content-between"><span>Remise</span><span>-{{ number_format($order->discount,0,' ',' ') }} </span></div> --}}
            <div class="d-flex justify-content-between"><span>Livraison</span><span>{{ number_format($order->shipping,0,' ',' ') }} </span></div>
            <div class="d-flex justify-content-between"><span>Taxes</span><span>{{ number_format($order->tax,0,' ',' ') }} </span></div>
          ...
          <tbody>
            @foreach($order->items as $it)
              <tr>
                ...
                <td class="text-end fw-semibold">{{ number_format($it->vendorTotal,0,' ',' ') }} </td>
              </tr>
            @endforeach
          </tbody>
        </table>
  ...
  <div class="col-lg-4">
    <div class="card mb-3">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('partners.shop.orders.status',$order) }}">
          @csrf
