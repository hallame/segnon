...
<div class="row mb-3">
    ...
    <div class="card flex-fill"><div class="card-body">
      <div class="d-flex align-items-center">
        <span class="p-2 br-10 bg-primary-transparent border border-primary me-2 d-flex align-items-center justify-content-center">
          <i class="ti ti-package fs-18 text-primary"></i></span>
        <div><p class="fs-12 fw-medium mb-1 text-gray-5">Total</p>
          <h4 class="mb-0">{{ $stats['total'] ?? 0 }}</h4></div>
      </div></div></div>
  </div>
    ...
    <div class="card flex-fill"><div class="card-body">
      <div class="d-flex align-items-center">
        <span class="p-2 br-10 bg-info-transparent border border-info me-2 d-flex align-items-center justify-content-center">
          <i class="ti ti-clock fs-18 text-info"></i></span>
        <div><p class="fs-12 fw-medium mb-1 text-gray-5">En attente</p>
          <h4 class="mb-0">{{ $stats['pending'] ?? 0 }}</h4></div>
      </div></div></div>
  </div>
    ...
    <div class="card flex-fill"><div class="card-body">
      <div class="d-flex align-items-center">
        <span class="p-2 br-10 bg-success-transparent border border-success me-2 d-flex align-items-center justify-content-center">
          <i class="ti ti-circle-check fs-18 text-success"></i></span>
        <div><p class="fs-12 fw-medium mb-1 text-gray-5">Payées</p>
          <h4 class="mb-0">{{ $stats['paid'] ?? 0 }}</h4></div>
      </div></div></div>
  </div>
  ...
  <div class="col-xl-3 col-md-6 d-flex">
    <div class="card flex-fill"><div class="card-body">
      <div class="d-flex align-items-center">
        <span class="p-2 br-10 bg-primary-transparent border border-primary me-2 d-flex align-items-center justify-content-center">
          <i class="ti ti-truck fs-18 text-primary"></i></span>
        <div><p class="fs-12 fw-medium mb-1 text-gray-5">Clôturées</p>
          <h4 class="mb-0">{{ $stats['fulfilled'] ?? 0 }}</h4></div>
      </div></div></div>
  </div>
</div>
...
<div class="card mb-3">
  <form class="card-body row g-3">
    <div class="col-md-4">
      <input type="text" name="q" value="{{ $q }}" class="form-control" placeholder="Réf / nom / email / téléphone">
    </div>
    <div class="col-md-2">
...
<div class="card">
  <div class="table-responsive">
    <table class="table align-middle mb-0">
      <thead><tr>
        <th>Réf</th><th>Client</th><th>Total ({{ $currency }})</th><th>Statut</th><th>Date</th><th>Actions</th>
      </tr></thead>
      <tbody>
      @forelse($orders as $o)
        @php
        ...
        @endphp
        <tr>
          <td><a href="{{ route('partners.shop.orders.show',$o) }}" class="fw-semibold">{{ $o->reference }}</a></td>
          <td>
            <div class="fw-semibold">{{ $o->customer_firstname }} {{ $o->customer_lastname }}</div>
            <div class="text-muted small">{{ $o->customer_email }}</div>
          </td>
          <td class="fw-semibold">{{ number_format($o->vendor_total , 0, ' ', ' ') }}</td>
          ...
          <td class="text-nowrap">
            <a href="{{ route('partners.shop.orders.show',$o) }}" class="btn btn-sm btn-outline-primary">Voir</a>
          </td>
        </tr>
      @empty
