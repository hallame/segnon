      ...
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
          ...
          @php
            ...
            $cls = $statusMap[$order->status] ?? 'bg-light text-dark';
          @endphp
          ...
          <tbody>
            @foreach($order->items as $it)
              <tr>
                ...
                <td class="text-end fw-semibold">{{ number_format($it->total_price,0,' ',' ') }} {{ $order->currency }}</td>
              </tr>
            @endforeach
          </tbody>
        </table>
  ...
  <div class="col-lg-4">
          ...
          <div class="d-flex gap-2">
            <select name="status" class="form-select">
              @foreach(\App\Models\Order::$statusLabels as $code=>$label)
                <option value="{{ $code }}" @selected($order->status===$code)>{{ ucfirst(str_replace('_',' ',$label)) }}</option>
              @endforeach
            </select>
    ...
    <div class="card">
      ...
      <div class="card-body small text-muted">
        ...
        <div>Client ID : {{ $order->client_id ?? '—' }}</div>
