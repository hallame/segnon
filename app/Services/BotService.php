...
class BotService {
    ...
    public function search(string $query): array {
        ...
        // VÉRIFICATION: Si la requête contient des mots génériques de produit
        $genericProductWords = ['produit', 'article', 'item', 'marchandise', 'produits', 'articles', 'items'];
        $hasGenericWord = false;

        foreach ($tokens as $token) {
            if (in_array($token, $genericProductWords)) {
                $hasGenericWord = true;
                break;
            }
        }

        // Si l'utilisateur tape un mot générique de produit, lui proposer directement les catégories
        if ($hasGenericWord) {
    ...
    private function detectIntent(string $query): ?string {
        $patterns = [
            ...
            'buy' => [
                ...
                '/\b(produits|articles|items|marchandises|biens|achats)\b/ui', // Pluriels
                '/\b(pc|laptop|ordinateur|téléphone|smartphone|tablette|vêtement|chaussure)\b/ui',
                '/\b(prix\s+de|coût\s+de|combien\s+coûte)\b/ui',
                '/\b(je\s+cherche|je\s+veux|trouver|rechercher|explorer)\b/ui', // Expressions générique
                ],
    ...
    private function suggestAlternatives(string $query, array $tokens): array {
        ...
        // Fallback si vraiment pas de catégories
        return [[
            'type' => 'Info',
            'title' => 'Aucun résultat',
            'excerpt' => "Désolé, je n'ai rien trouvé pour '" . e($query) . "'",
    ...
    private function handleDirectQuery(string $query, string $queryLower): array {
        ...
        if (empty($tokens)) {
            return [[
                ...
                'excerpt' => "Exemples:<br>'Je cherche un téléphone'<br>'Voir les produits'",
                'url' => route('bot'),
            ]];
        }
    ...
    private function searchProducts(array $keywords): array {
        ...
        if ($items->isEmpty()) {
            return [[
                ...
                'excerpt' => 'Essayez notre boutique: <a href="' . route('shop.products.index') . '">Parcourir les produits</a>',
                'url' => route('bot'),
            ]];
        }
    ...
    private function unifyResults(string $sourceKey, iterable $items, ?string $labelOverride = null): array {
        ...
        foreach ($items as $it) {
            ...
            $extra = [];
            // Pour les produits populaires avec views_count
            if (property_exists($it, 'views_count') || isset($it->views_count)) {
                $extra['views_count'] = $it->views_count ?? 0;
            }

            // Pour les relations chargées
            if ($it->relationLoaded('views')) {
                $extra['views_count'] = $it->views->count();
            }
    ...
    private function dynamicOptionsForIntent(string $intent, array $conf): array {
     ...
     private function getCategoriesForProducts(): array {
        $categories = \App\Models\Category::query()
            ->where('model', 'Product')
            ->where('status', 1)
            ->withCount(['activeProducts as products_count']) // Compte les produits actifs
            ->having('products_count', '>', 0) // Seulement les catégories avec produits
            ->orderBy('name')
            ->limit(12)
            ->get(['id', 'name', 'slug']);
        ...
        foreach ($categories as $category) {
            ...
            $options[] = [
                'display' => $category->name . " ({$category->products_count})", // Ajouter le comptage
                'value' => $slug,
                'id' => $category->id
            ];
        }
