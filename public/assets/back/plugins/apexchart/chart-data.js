...
if ($('#plan-overview').length > 0) {
  ...
  if ($('#leads_stage').length > 0) {
    ...
    if ($('#donut-chart-2').length > 0) {
        var chartLabels = window.chartLabels || ['Ongoing', 'Onhold', 'Completed', 'Overdue', 'Rejected'];
        var chartData = window.chartData || [20, 30, 30, 10, 10];
        var chartDatasetLabel = window.chartDatasetLabel || '-0-'; // Label par défaut
        var options = {
        series: chartData, // Percentages for each section
        chart: {
            type: 'donut',
            height: 185,
        },
        labels: chartLabels, // Labels for the data
        colors: ['#0C4B5E', '#FFC107', '#007521', '#AB47BC', '#FF0000'], // Colors from the image
        plotOptions: {
            pie: {
                donut: {
                    size: '60%',
        ...
        width: 0,    // Space between donut sections
        // colors: '#fff'
        colors: ['#fff'] // Doit être un tableau
        },
            legend: {
            show: false,
            },
    ...
    if ($('#reservations').length > 0) {
        var chartLabels = window.chartLabels || ['Confirmés', 'En Attente', 'Annulés'];
        var chartData = window.chartData || [20, 30, 50];
        var chartDatasetLabel = window.chartDatasetLabel || '-0-'; // Label par défaut
        var options = {
            series: chartData, // Percentages for each section
            chart: {
            ...
            labels: chartLabels, // Labels for the data
            colors: ['#007521', '#FFC107', '#ff070f'], // Colors from the image
            plotOptions: {
                pie: {
                    donut: {
                        size: '60%',
