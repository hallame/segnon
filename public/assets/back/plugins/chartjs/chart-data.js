...
if ($('#chartDonut').length > 0) {
	...
	if($('#mySemiDonutChart').length > 0) {
        ...
        // Valeurs par défaut si elles ne sont pas définies dans la page
        
        var chartLabels = window.chartLabels || ['Ongoing', 'Onhold', 'Completed', 'Overdue', 'Rejected'];
        var chartData = window.chartData || [20, 30, 20, 20, 10];
        var chartDatasetLabel = window.chartDatasetLabel || 'Semi Donut'; // Label par défaut
        var mySemiDonutChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: chartData,
                    backgroundColor: ['#FFC107', '#1B84FF', '#03C95A', '#E70D0D', '#FF1493'],
                    borderWidth: -10,
